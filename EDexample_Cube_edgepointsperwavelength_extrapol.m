% EDexample_Cube_edgepointsperwavelength_extrapol.m
%
% This script computes the sound pressure in two receiver positions near a
% cube, for plane wave incidence. Twenty frequencies, and nine different
% numbers of gausspoints per edge are run. 
% 
% Extrapolated results are generated for each of the two last results.
% Their error are computed with the best extrapolated result as reference.
...
ylabel('Rel. error   [-]')
title('Rel. error for extrapol. result, plane wave incidence onto a cube (ref. = best result)')
ylim([1e-7 0.1])
